        ...
        for frame_idx, frame in enumerate(sampled_frames):
            ...
            for obj_id in sampled_object_ids:
                ...
                try:
                    ...
                    logging.warning(f"Error loading segment for object {obj_id}: {e}")
                    segment = torch.zeros(h, w, dtype=torch.uint8)  # Fallback to empty mask
...
def load_images(frames):
    ...
    for frame in frames:
        try:
            ...
            logging.warning(f"Error loading image from {frame.image_path}: {e}")
            # Continue to the next frame if an error occurs
            continue
